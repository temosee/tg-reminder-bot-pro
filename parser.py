...
WORD_NUMBERS = {
    ...
    "миллион": 1000000, "миллиона": 1000000,
...
UNITS_TO_SECONDS = {
    ...
    "сутки": 86400, "суток": 86400, "сутей": 86400,
...
TIME_OF_DAY = {
    ...
    "на завтрак": 8, "завтрак": 8, "завтрака": 8,
    "днём": 13, "днем": 13, "в обед": 13, "обед": 13, "обеда": 13, "на обед": 13,
    "вечером": 17, "вечера": 17, "вечерком": 17, "ближе к вечеру": 18,
    "на ужин": 19, "ужин": 19, "ужина": 19,
    "перед сном": 22, "ночью": 22, "ночи": 22,
...
_ALL_UNITS = (
    ...
    r"|дн[яейи]|день|дней|сутки?|суток|недел[юьие]|недель|месяц(?:ев|а|е)?|год|года|лет"
)
...
def _normalize(text: str) -> str:
    ...
    subs = [
        ...
        (r"\bполминут[ыи]?\b", "30 секунд"),
        (r"\bпол\s+минут[ыи]?\b", "30 секунд"),
        (r"\bполовин[уы]\s+час[аик]*\b", "30 минут"),
        (r"\bчетверт[ьи]\s+час[аик]*\b", "15 минут"),
        (r"\bчетверт[ьи]\s+минут[ыи]?\b", "15 секунд"),
        (r"\b(\d+),(\d+)\b", r"\1.\2"),
        (r"\bдва\s+с\s+половиной\b", "2.5"),
        (r"\bтри\s+с\s+половиной\b", "3.5"),
        (r"\bчетыре\s+с\s+половиной\b", "4.5"),
        (r"\bпять\s+с\s+половиной\b", "5.5"),
...
def _parse_interval(text: str, tz: zoneinfo.ZoneInfo = None) -> int | None:
    ...
    if re.search(r"каждую\s+секунду", text):
        return 1
    if re.search(r"каждый\s+день|каждые?\s+сутки|ежедневно", text):
        return 86400
    if re.search(r"каждую\s+неделю", text):
        return 604800
    if re.search(r"каждый\s+месяц|ежемесячно", text):
        return 2592000
    if re.search(r"каждый\s+год|ежегодно", text):
        return 31536000
    ...
    m_raz_solo = re.search(
        r"раз\s+в\s+(час|минуту|день|сутки|неделю|месяц|год)\b",
        text
    )
    if m_raz_solo:
    ...
    m_raz = re.search(
        r"раз\s+в\s+([\wа-яё]+)\s+(секунд[у-ы]?|сек|минут[у-ы]?|мин|час[а-ов]*|ч|дн[яейи]|день|дней|сутки?|недел[юьи]|недель|месяц(?:ев|а|е)?|год|года|лет)\b",
        text
    )
    if m_raz:
...
def _parse_once_delta(text: str, tz: zoneinfo.ZoneInfo = None) -> float | None:
    ...
    m_day_short = re.search(r'\b(\d{1,2})-го\b(?!\s+числ)', text)
    if m_day_digit:
        _day_num = int(m_day_digit.group(1))
    elif m_day_word:
        _day_num = _ORDINAL_TO_DAY[m_day_word.group(1)]
    elif m_day_short:
        _day_num = int(m_day_short.group(1))
    ...
    if re.search(r"через\s+(?:день|сутки)\b", text):
    ...
    if re.search(r"через\s+год\b", text):
        return time.time() + 31536000
    ...
    def _resolve_unit(unit: str, num: float) -> float | None:
        ...
        if mult:
            ...
            if target_ts > 4102444800:  # > year 2100
                return target_ts  # будет отфильтровано лимитом в parse()
            if mult >= 86400:
...
_TOD_PAT = r"утром|утра|с\s+утра|днём|днем|в\s+обед|обед[а]?|вечером|вечера|вечерком|ближе\s+к\s+вечеру|ночью|ночи|в\s+полночь|полночь|полуночи|в\s+полдень|полдень|полудня|на\s+завтрак|на\s+обед|на\s+ужин|перед\s+сном|завтрак[а]?|ужин[а]?"
_WEEKDAY_STRIP = (
...
_DELTA_STRIP = (
    ...
    rf"|\b(?:{_ORDINAL_DAY_NAMES})\s+(?:{_MONTH_NAMES_RU})\b"      # первого мая
    r"|\b\d{1,2}-го\b"                                              # 15-го
...
def _extract_message(text: str) -> str:
    ...
    result = re.sub(r"\b(пожалуйста|плз|плиз|плс|блин|бля|ну|вот|типа|короч|короче|кстати|просто|давай|ровно|примерно|около|где-то|че\s+там|чё\s+там|ой|начиная)\b", "", result, flags=re.IGNORECASE)

    trigger_pat = re.compile(
        r"^(.*?)\b(напомни|напоминай|напомнить|напомнил[аи]?|дай\s+знать|нужно\s+напомнить|скажи|смотри|слушай)\b[-\s]*(ка\b\s*)?\s*(мне\s*)?(пожалуйста\s*)?"
        r"(?:\s*(?:напомни|напоминай|напомнить)\s*(?:мне\s*)?)?",
        re.IGNORECASE
    )
...
def parse(text: str, user_tz: str | None = None) -> dict:
    ...
    is_recurring = bool(re.search(
        ...
        r"напоминалку каждый|напоминалку каждые|ежедневно|ежечасно|ежемесячно|ежегодно",
        lower
    ...
    if is_recurring:
        ...
        msg = re.sub(
            r"каждый\s+час|каждую\s+минуту|каждую\s+секунду|каждый\s+день|каждые?\s+сутки|каждую\s+неделю|каждый\s+месяц|каждый\s+год|ежедневно|ежечасно|ежемесячно|ежегодно|"
            r"раз\s+в\s+(?:час|минуту|день|сутки|неделю|месяц|год)\b|"
            r"раз\s+в\s+[\wа-яё]+\s+(?:секунд[у-ы]?|сек|минут[у-ы]?|мин|час[а-ов]*|ч|дн[яейи]|день|дней|сутки?|недел[юьи]|недель|месяц(?:ев|а|е)?|год|года|лет)\b|"
    ...
    is_once = bool(re.search(
        ...
        r"дай знать|скажи\b|смотри\b|слушай\b",
        lower
    ))
